...
import {
  ...
  createJornada, closeJornada, getJornada, linkMatchToJornada,
  computeSeasonAwards, getSeasonAwards, getMatches,
} from '../api/client'
import { Trophy, Calendar, Plus, X, Award, ChevronDown, ChevronUp, Link, Clock } from 'lucide-react'
  ...
  async function load() {
    ...
    try {
      const [active, all, matchList] = await Promise.all([
        getActiveSeason(), getSeasons(), getMatches(),
      ])
                    ...
                    <div className="px-3 pb-3 border-t border-surface-700/30 pt-2">
                      {jornadaData[j.id].matches?.length > 0 ? (
                        <div className="space-y-1">
                          ...
                          <p className="text-[10px] text-surface-500 mt-1">{jornadaData[j.id].matches.length} match(es) linked</p>
                        </div>
                      ) : (
            ...
            <div className="bg-surface-900/40 rounded-xl border border-surface-700/20 p-3 space-y-2 mt-3">
              ...
              {jornadaStart && jornadaEnd && (
                <p className="text-[10px] text-accent/80">
                  Matches played between {jornadaStart} and {jornadaEnd} will be auto-linked to this jornada.
                </p>
              )}
            </div>

            {/* Manual link match to jornada */}
